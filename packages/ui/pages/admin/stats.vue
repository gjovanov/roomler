            ...
            <v-row>
              <v-col cols="12">
                ...
                <v-data-table
                  ...
                  :server-items-length="visitStatsCount"
            ...
            <v-row>
              <v-col cols="12">
                ...
                <v-data-table
                  ...
                  :server-items-length="callStatsCount"
...
export default {
  async asyncData ({ store, query }) {
    ...
    delete q2.pageCalls
    q2.page--
    if (query.panel && (parseInt(query.panel) === 1 || parseInt(query.panel) === 3)) {
      await store.dispatch('api/visit/getStats', q1)
    }
    if (query.panel && (parseInt(query.panel) === 2 || parseInt(query.panel) === 4)) {
      store.dispatch('api/room/calls/getStats', q2)
    }
  },
  data () {
    const from = new Date()
    ...
    const filter = {
      ...
      type: this.$route.query.type || 'chart',
      ...
      panel: this.$route.query.panel !== undefined ? (this.$route.query.panel !== null ? parseInt(this.$route.query.panel) : 0) : null,
    ...
    if ([1, 2].includes(filter.panel)) {
      filter.type = 'grid'
    }
    if ([3, 4].includes(filter.panel)) {
      filter.type = 'chart'
    }
    return {
      panels: 0,
  ...
  computed: {
    ...
    visitStatsLoading () {
      return this.$store.state.api.visit.statsLoading
    },
    visitStats () {
      return this.$store.state.api.visit.stats
    },
    visitStatsCount () {
      const count = Math.ceil(parseFloat(this.$store.state.api.visit.statsCount) / this.filter.size)
      return count
    },
    callStatsLoading () {
      return this.$store.state.api.room.calls.statsLoading
    },
    callStats () {
      return this.$store.state.api.room.calls.stats
    },
    callStatsCount () {
      const count = Math.ceil(parseFloat(this.$store.state.api.room.calls.statsCount) / this.filter.size)
      return count
    }
  },
  watch: {
  ...
  watchQuery: ['from', 'to', 'status', 'room', 'country', 'user', 'os', 'browser', 'url', 'referrer', 'pageVisits', 'pageCalls', 'size', 'sortBy', 'sortDesc', 'interval', 'panel']
}
</script>
